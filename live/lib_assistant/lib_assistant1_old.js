...
function s61SessionTracker(linkType) {
    if (s61Valid) {
        var t1 = new Date().getTime(), t2, isPageVisited = false,
                totalDuration = 0, server, origin = "*", ms = t1 - 604800000/* 1 week */,
                pageId, intervalId, url = location.pathname, hashId = hashCode(url);
        ...
        db.open({
            ...
            version: 1,
            schema: {
                link: {key: {keyPath: 'id', autoIncrement: true}, indexes: {hashId: {unique: true}}},
            ...
            if (ti > 3) {//5 seconds is minimum valuable user visit that i could to count that handled by interval function
                ...
                if (pageId) {
                    ...
                    server.visit.add(obj).then(function (i) {
                        i = i[0];
                        pageId = i.id;
//                    console.log("this is persisted :", i.id, i.hashId, i.type, i.date, i.duration)
                    });
                }
            }
        }
...
//                    console.log(getVisitStatistics.name, "link", item);
                        server.visit.query().filter('hashId', item.hashId).execute().catch(function () {
                            len++;
                            if (len >= result.length)
                                callBack(arr)
//                        console.log(getVisitStatistics.name, " catch , len: ", len);
                        }).then(function (value) {
//                        console.log(getVisitStatistics.name, "all hashId values", "hashId: " + item.hashId, "values: ", value);
...
//[[url1,quantity of organic,organic total duration,organic maximum duration,quantity of profile,profile total duration,profile maximum duration],[]]
                                arr.push([item.hashId, item.url, cOrganic, dOrganic, maxDOrganic, cProfile, dProfile, maxDProfile]);
                            }
                            len++;
    ...
    if (window.self === window.top && window.top.frames.length > 0) {
        ...
        window.addEventListener('message', function (e) {
            ...
            if (e.origin === s61y) { //*** change origin url to my site
                ...
                if (e.c === "url") {
                    pm({c: "url", isvalid: s61Valid, "path": location.pathname});
