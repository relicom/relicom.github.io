...
var s61Ver = 1, //s61linkTypeEnum = Object.freeze({"organic": 1, "profile": 2}), 
        ...
        s61w = s61Select("#s61w"), s61sId = -1, // Number(s61w.dataset.numid),
...
function s61Init() {
    ...
    s61css.href = s61b + "lib_assistant_localstorage.css";
...
//        console.log(persistNewVisit.name, ti);
            if (ti > 3) {//3 seconds is minimum valuable user visit that i could to count that handled by interval function
                var obj = s61ls.getItem(s61ProductPrefix + s61hash);
//                console.log("persistNewVisit +++++ ",obj,s61Prefix + s61hash,localStorage.getItem(s61Prefix + s61hash));
                ...
                s61ls.setItem(s61ProductPrefix + s61hash, obj);
            }
        }
...
function s61ProductDetail() {
    var str = '{"link":"' + location.origin + location.pathname + '","hash":' + s61hash + ',"title":"' + s61ti + '","img":"' + s61im.src + '","numid":' + s61sId + '}';
...
////    s61VisitStatistics();
//}, 3000);
////*** s61VisitStatistics() has complete version in live_participant.js
//function s61VisitStatistics() {
//    var arr = [], k, v, j = {};
//    var keys = Object.keys(s61ls);
//    for (var b = 0; b < keys.length; b++) {
//        k = keys[b];
//        if (k.startsWith(s61ProductPrefix)) {
...
//        }
//        if (totalTime > 0) {
//            arr.push([k.substr(s61ProductPrefix.length), j[k].url, j[k].title, j[k].img, totalTime]);
//        }
//    }
