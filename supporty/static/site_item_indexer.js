if (window.top === window.self) {
        var body = g('body'), LINK_LIMIT = 2, o = "http://localhost:8383", ranKey = "ewinyeuixneywuy3213189ns12yxb812ns28nx12fsdfs",
                siteId = 1234, titleSelector = "#titr", imgSelector = "#item-pic",
        ...
        sendBt.onclick = function () {
            ...
            var req = {key: ranKey, siteid: siteId, data: b64EncodeUnicode(JSON.stringify(array))};
            hide(sendBt, 1);
        ...
        function init() {
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    if (xhr.status === 200) {
                        reset();
                        storedLinks = JSON.parse(xhr.responseText);
                        if (storedLinks) {
